...
- groupKey는 가능한 한 "공종-대분류-소분류" 형식을 따르세요.
    ...
    allGroupedData.forEach(item => {
      const normalizedGroupKey =
        typeof item.groupKey === "string" && item.groupKey.trim().length > 0
          ? item.groupKey.trim()
          : `${item.trade || "기타"}-${item.majorCategory || "기타"}-${item.minorCategory || "기타"}`;

      const [trade = "기타", majorCategory = "기타", minorCategory = "기타"] = normalizedGroupKey.split("-");
      ...
      const key = `${trade}-${majorCategory}-${minorCategory}`;
      if (finalMergeMap[key]) {
        // 이미 있으면 variations 배열만 합쳐주고 중복제거
        finalMergeMap[key].variations = [...finalMergeMap[key].variations, ...(normalizedItem.variations || [])];
        finalMergeMap[key].removeCandidates = [
          ...(finalMergeMap[key].removeCandidates || []),
          ...(normalizedItem.removeCandidates || [])
        ];
        if (!finalMergeMap[key].representativeContent && normalizedItem.representativeContent) {
          finalMergeMap[key].representativeContent = normalizedItem.representativeContent;
          finalMergeMap[key].representativeSource = normalizedItem.representativeSource;
          finalMergeMap[key].reason = normalizedItem.reason;
        }
      } else {
        finalMergeMap[key] = normalizedItem;
      }
    });
